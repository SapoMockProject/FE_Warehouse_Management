...
const getSelectedLabel = () => {
  ...
  const selectedChildren = childArray.filter((child: any) =>
    React.isValidElement(child) && (value as string[]).includes(child.props.value)
  ) as any[];
    ...
    <div className={`custom-select ${className}`} ref={dropdownRef}>
      {renderTrigger ? (
        <div onClick={() => !disabled && setIsOpen(!isOpen)}>
          {renderTrigger(getSelectedLabel(), isOpen)}
        </div>
      ) : (
        <div
