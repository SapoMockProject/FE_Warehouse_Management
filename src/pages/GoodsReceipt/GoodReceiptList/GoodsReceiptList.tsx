...
const GoodsReceiptList: React.FC = () => {
	...
	const fetchReceipts = async () => {
		...
		try {
			const receiptStatus = state.selectedReceiptStatus === "all" ? undefined : state.selectedReceiptStatus === "received";

			const transactionStatus = state.selectedTransactionStatus === "all" ? undefined : state.selectedTransactionStatus;
	...
	const getTransactionStatusLabel = (status: string) => {
		const statusMap: Record<string, string> = TRANSACTION_STATUSES.reduce((acc, item) => ({ ...acc, [item.value]: item.label }), {});
	...
	const handleFilterChange = <K extends keyof GoodsReceiptListState>(field: K, value: GoodsReceiptListState[K]) => {
	...
	const handleStateChange = <K extends keyof GoodsReceiptListState>(field: K, value: GoodsReceiptListState[K]) => {
					...
					<div className="opr-filter-bar">
						...
						<div style={{ flex: "0 1 auto" }}>
							<DateField
								...
								onChange={(e) => handleFilterChange("dateRange", e as DateRange)}
							/>
						</div>

						<div style={{ flex: "0 1 auto" }}>
							<StatusSelect
								value={state.selectedReceiptStatus === "all" ? [] : [state.selectedReceiptStatus]}
								onChange={(values) => {
									if (values.length === 0) {
						...
						<div style={{ flex: "0 1 auto" }}>
							<StatusSelect
								value={state.selectedTransactionStatus !== "all" ? [state.selectedTransactionStatus] : []}
								onChange={(statuses) => {
									...
									<tbody>
										{filteredReceipts.map((item) => (
											...
											>
												...
												<td className="td_highlight">
													<a href={`/suppliers/${item.supplier.id}`} onClick={(e) => e.stopPropagation()}>
														{item.supplier?.name || "N/A"}
													</a>
												</td>
												<td>{item.infoEmployeeIsAssigned?.fullName || "N/A"}</td>
												<td className="align_center">{getTotalQuantity(item.items)}</td>
