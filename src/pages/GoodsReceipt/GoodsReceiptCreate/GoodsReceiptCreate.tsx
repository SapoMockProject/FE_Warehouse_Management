...
const GoodsReceiptCreate: React.FC = () => {
	...
	const purchaseOrderData = location.state?.purchaseOrderData as (PurchaseOrderResponse | null);
	...
	useEffect(() => {
		if (purchaseOrderData) {
			...
			const convertedItems: ProductVariantItem[] = purchaseOrderData.items.map((item) => {
				...
				return {
					...
					discountValue: item.discountValueItem || undefined,
	...
	useEffect(() => {
		...
		const totalLineItemsPriceAfterDiscount = goodsReceiptRequest.items.reduce((sum, item) => sum + item.subtotalPriceItem, 0);
		const totalDiscountValue = totalItemsDiscount;

		const totalLandedCost = goodsReceiptRequest.totalLandedCost || 0;

		const totalPrice = Math.max(0, totalLineItemsPriceAfterDiscount + totalLandedCost);
	...
	const updateQuantity = (variantId: number, quantity: number | string) => {
		...
		setOrderItems(orderItems.map((item) => (item.id === variantId ? { ...item, quantityPurchase: qty } : item)));
	...
	const handleChangeGoodsReceiptField = <K extends keyof GoodsReceiptRequest>(field: K, value: GoodsReceiptRequest[K]) => {
		setGoodsReceiptRequest((prev) => ({
			...prev,
			[field]: value,
		}));
	...
	const handlePaymentFieldChange = (field: keyof TransactionRequest, value: string | number | null | undefined) => {
		setGoodsReceiptRequest((prev) => ({
			...prev,
	...
	const handleSubmitGoodsReceipt = async () => {
		...
		const accountId = goodsReceiptRequest.assignedToAccountId ? goodsReceiptRequest.assignedToAccountId : user?.user.id || null;
		...
		<div className="purchase-order-page">
			<div className="purchase-order-header">
				...
				<h1 className="purchase-order-title">
					{purchaseOrderData ? `Nhập hàng từ đơn đặt #${purchaseOrderData.purchaseOrderCode}` : "Tạo phiếu nhập hàng"}
				</h1>
			</div>
								...
								{isOpenSearchVariant && (
									<div className="purchase-order-dropdown">
										<div className="purchase-order-dropdown-item">
											...
											>
												{loadingVariant ? "" : hasMoreProduct ? "Cuộn để tải thêm" : ""}
											</div>
										</div>
									</div>
								)}
							...
							{!showPaymentForm ? (
								...
								<div className="payment-details-form">
									...
									<div className="purchase-order-form-group">
										...
										>
											<Button
												label="25%"
												onClick={() =>
													handlePaymentFieldChange("amount", Math.round(goodsReceiptRequest.totalPrice * 0.25))
												}
												variant="tertiary"
												size="sm"
											/>
											<Button
												label="50%"
												onClick={() =>
													handlePaymentFieldChange("amount", Math.round(goodsReceiptRequest.totalPrice * 0.5))
												}
												variant="tertiary"
												size="sm"
											/>
											<Button
												label="75%"
												onClick={() =>
													handlePaymentFieldChange("amount", Math.round(goodsReceiptRequest.totalPrice * 0.75))
												}
												variant="tertiary"
												size="sm"
											/>
									...
									<div className="purchase-order-form-group">
										...
										/>
										{error.processedOn && <ValidationMessage show={true} message={error.processedOn} type="error" />}
									</div>
									...
									<div className="payment-summary-box">
										...
										<div className="payment-summary-row">
											...
											>
												{(
													goodsReceiptRequest.totalPrice - (goodsReceiptRequest.transactionInfo?.amount || 0)
												).toLocaleString("vi-VN")}
												đ
											</strong>
										</div>
									...
									{isOpenSearchSupplier && (
										<div className="purchase-order-dropdown">
											...
											<div ref={observerSupplierRef} style={{ textAlign: "center", padding: "10px" }}>
												{loadingSupplier ? "Đang tải..." : hasMoreSupplier ? "Cuộn để tải thêm" : ""}
											</div>
										</div>
									)}
