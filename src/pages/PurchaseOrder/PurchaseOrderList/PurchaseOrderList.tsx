...
export default function PurchaseOrderRequest() {
	...
	const getStatusLabel = (status: string) => {
		const statusMap: Record<string, string> = PURCHASE_ORDER_STATUSES.reduce((acc, item) => {
	...
	const getStatusClass = (status: string) => {
		const classMap: Record<string, string> = PURCHASE_ORDER_STATUSES.reduce((acc, item) => {
					...
					<div className="opr-filter-bar">
						...
						<div style={{ flex: "0 1 auto" }}>
							<DateField
								...
								onChange={(e) => handleStateChange("dateRange", e as DateRange)}
							/>
						</div>
						...
						<div style={{ flex: "0 1 auto" }}>
							<StatusSelect
								...
								onChange={(statuses) => handleStateChange("selectedStatuses", statuses)}
