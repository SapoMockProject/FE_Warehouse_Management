...
export default function UpdateProduct() {
	...
	const getVariantName = (product: ProductResponse, variant: ProductVariant): string => {
	...
	const applyNewSku = async () => {
		...
		try {
			const token = localStorage.getItem("token");
			const res = await axios.put("http://localhost:8080/api/v1/product-variant/update-skus", updateList, {
				headers: {
					Authorization: `Bearer ${token}`,
					"Content-Type": "application/json",
				},
	...
	const applyNewPrice = async () => {
		...
		try {
			const token = localStorage.getItem("token");
			const res = await axios.put("http://localhost:8080/api/v1/product-variant/update-prices", updateList, {
				headers: {
					Authorization: `Bearer ${token}`,
					"Content-Type": "application/json",
				},
	...
	const applyNewStock = async () => {
		...
		try {
			const token = localStorage.getItem("token");
			const res = await axios.put("http://localhost:8080/api/v1/product-variant/update-stocks", updateList, {
				headers: {
					Authorization: `Bearer ${token}`,
					"Content-Type": "application/json",
				},
			...
			<div className="update-product-grid">
							...
							<thead>
								{selected.length > 0 ? (
									<tr>
										...
										<th>
											<Button label="Sửa SKU" onClick={() => setIsModalOpenSKU(true)} />
											{isModalOpenSKU && (
												<div className="update-variant-sku-modal-overlay">
													<div className="update-variant-sku-modal-box">
															...
															.map((e) => (
																<div className="update-variant-sku" key={e.id}>
																	<div className="variant-sku-lable">
																		<label>
																			{[e.option1value, e.option2value, e.option3value]
																				.filter(Boolean)
																				.join(" / ")}
																		</label>
																	</div>
														...
														<div className="update-product-modal-actions">
															<Button label="Hủy" size="md" onClick={() => setIsModalOpenSKU(false)} />

															<Button label="Áp dụng" variant="secondary" size="md" onClick={applyNewSku} />
														</div>
													</div>
												</div>
											)}
										...
										<th>
											<Button label="Sửa Giá" onClick={() => setIsModalOpenPrice(true)} />
											{isModalOpenPrice && (
													...
													<div className="update-variant-price-modal-box">
														...
														<div className="update-all-variant-price">
															...
															<div className="variant-price-right">
																<Button label="Áp dụng cho tất cả" variant="tertiary" size="lg" />
															</div>
														</div>
														{product?.variants
															.filter((e) => selectedArray.includes(e.id))
														...
														<div className="update-product-modal-actions">
															<Button label="Hủy" size="md" onClick={() => setIsModalOpenPrice(false)} />

															<Button label="Áp dụng" variant="secondary" size="md" onClick={applyNewPrice} />
														</div>
													</div>
												</div>
											)}
										...
										<th>
											<Button label="Sửa số lượng" onClick={() => setIsModalOpenStock(true)} />
											{isModalOpenStock && (
													...
													<div className="update-variant-stock-modal-box">
														...
														<div className="update-all-variant-stock">
															<div className="variant-stock-left">
																<p>Áp dụng một số lượng cho tất cả các phiên bản</p>
																<Input
																	type="text"
																	value={stock}
														...
														{product?.variants
															.filter((e) => selectedArray.includes(e.id))
															.map((e) => (
																<div className="update-variant-stock" key={e.id}>
														...
														<div className="update-product-modal-actions">
															<Button label="Hủy" size="md" onClick={() => setIsModalOpenStock(false)} />

															<Button label="Áp dụng" variant="secondary" size="md" onClick={applyNewStock} />
														</div>
													</div>
												</div>
											)}
				...
				<div>
					<div className="update-product-card">
						...
						<div className="update-product-image-box" onClick={() => fileRef.current?.click()}>
							<div>
								<div style={{ fontSize: 18, marginBottom: 8 }}>+ Kéo thả hoặc thêm ảnh</div>
								<div className="update-product-muted">Dung lượng tối đa 4MB, tối đa 9 ảnh</div>
							</div>
						...
						<div className="update-product-helper-row">
							<div className="update-product-small add-product-muted">Kéo thả ảnh hoặc nhấn để chọn</div>
							<div className="update-product-small add-product-muted">{files.length} ảnh</div>
						</div>
					</div>
					...
					<div className="update-product-card" style={{ marginTop: 20 }}>
						...
						<CustomSelect
							placeholder={
								categories ? allCategories.find((c) => c.id.toString() === categories)?.name : product?.categoryName
							}
							value={categories}
